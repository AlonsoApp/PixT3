keywords = ['only', 'and', 'greater', 'less', 'eq', 'not_eq', 'round_eq', 'all_eq',
            'all_not_eq', 'all_less', 'all_less_eq', 'all_greater', 'all_greater_eq',
            'most_eq', 'most_not_eq', 'most_less', 'most_less_eq', 'most_greater',
            'most_greater_eq', 'all_rows', 'filter_eq', 'filter_not_eq',
            'filter_less', 'filter_greater', 'filter_greater_eq', 'filter_less_eq', 'filter_all', 'count', 'avg', 'sum',
            'max', 'min', 'nth_max', 'nth_min', 'argmax', 'argmin', 'nth_argmax', 'nth_argmin', 'hop', 'hop_first',
            'diff', 'contains', 'not_contains', 'all_contains', 'all_not_contains',
...
class Stat(Action):
    ...
    id_not_contains = 20
    id_all_contains = 21
    id_all_not_contains = 22
    id_most_contains = 23
    id_most_not_contains = 24
    ...
    def potential_case_value(self):
        if self.id_c in [Stat.id_greater, Stat.id_less, Stat.id_all_less, Stat.id_all_less_eq,
                         ...
                         Stat.id_most_greater, Stat.id_most_greater_eq, Stat.id_not_eq,
                         Stat.id_all_not_eq, Stat.id_not_contains, Stat.id_all_not_contains]:
            return V.AUX
        elif self.id_c in [Stat.id_all_eq, Stat.id_most_eq, Stat.id_most_not_eq, Stat.id_most_contains,
                           Stat.id_most_not_contains, Stat.id_all_contains]:
            return V.TAB  # Former TAB2
        elif self.id_c in [Stat.id_eq, Stat.id_round_eq, Stat.id_contains]:
...
class View(Action):
    ...
    id_filter_less_eq = 6
    id_filter_all = 7
    id_filter_contains = 8
    id_filter_not_contains = 9
    ...
    def potential_case_value(self):
        if self.id_c in [View.id_filter_less, View.id_filter_greater, View.id_filter_greater_eq, View.id_filter_less_eq]:
            return V.AUX
        elif self.id_c in [View.id_filter_eq, View.id_filter_not_eq, View.id_filter_contains,
                           View.id_filter_not_contains]:
            return V.TAB  # Former TAB2
        else:
            return None
...
class N(Action):
    ...
    def potential_case_value(self):
        ...
        return V.INF if self.id_c in [N.id_count, N.id_avg, N.id_sum] else V.TAB
...
class Obj(Action):
    ...
    def potential_case_value(self):
        ...
        elif self.id_c in [Obj.id_hop, Obj.id_hop_first, Obj.id_substring, Obj.id_substring_range]:
            return V.TAB
